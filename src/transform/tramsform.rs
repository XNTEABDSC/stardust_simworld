...
use bevy::{app::{App, Update}, ecs::system::{Query, Res}, math::{Mat3, Quat, Vec3}, time::{Fixed, Time}, transform::components::Transform};
use frunk::HList;
...
pub fn world_pos_to_transform_3d<Num:RealField+Copy>(mut q:Query<(&Stat<Pos<Num,2>>, &Stat<Vel<Num,2>>, &mut Transform, &Stat<TimePass<Num>>)>,fixed_time:Res<Time<Fixed>>){
	...
	q.par_iter_mut().for_each(|(q,v,mut trans,dt_per_frame)|{
		...
		trans.translation=pos_v+vec_v*dt;
	});
}
...
pub fn plugin_3d<Num:RealField+Copy>(app:&mut App){
	app.add_systems(Update, (world_rot_to_transform_3d::<Num>,world_rot_to_transform_3d::<Num>));
}
pub fn plugin_2d<Num:RealField+Copy>(app:&mut App){
	app.add_systems(Update, (world_rot_to_transform_2d::<Num>,world_rot_to_transform_2d::<Num>));
}